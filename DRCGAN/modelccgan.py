...
def build_generator_unets(inputgen, dim, numofres=3, name="generator"):
    with tf.variable_scope(name):
        f = 7
        ks = 3

        pad_input = tf.pad(inputgen, [[0, 0], [ks, ks], [ks, ks], [ks, ks], [0, 0]], "REFLECT")
        o_c1_1 = general_conv3d(pad_input, dim, f, f, f, 1, 1, 1, 0.02, "VALID", name="c1_1")
        o_c1_2 = tf.nn.max_pool3d(o_c1_1, [1, ks, ks, ks, 1], [1, 2, 2, 2, 1], "SAME", name="c1_2")

        o_c2_1 = general_conv3d(o_c1_2, dim*2, ks, ks, ks, 1, 1, 1, 0.02, "SAME", name="c2_1")
        o_c2_2 = tf.nn.max_pool3d(o_c2_1, [1, ks, ks, ks, 1], [1, 2, 2, 2, 1], "SAME", name="c2_2")

        o_c3_1 = general_conv3d(o_c2_2, dim*4, ks, ks, ks, 1, 1, 1, 0.02, "SAME", name="c3_1")
        o_c3_2 = tf.nn.max_pool3d(o_c3_1, [1, ks, ks, ks, 1], [1, 2, 2, 2, 1], "SAME", name="c3_2")

        o_c4_1 = general_conv3d(o_c3_2, dim*8, ks, ks, ks, 1, 1, 1, 0.02, "SAME", name="c4_1")
        o_c4_2 = tf.nn.max_pool3d(o_c4_1, [1, ks, ks, ks, 1], [1, 2, 2, 2, 1], "SAME", name="c4_2")

        o_c5_1 = general_conv3d(o_c4_2, dim*16, ks, ks, ks, 1, 1, 1, 0.02, "SAME", name="c5_1")

        o_c4_d = general_deconv3d(o_c5_1, dim*8, ks, ks, ks, 2, 2, 2, 0.02, "SAME", "c4_d")
        o_c4_3 = general_conv3d(tf.concat((o_c4_d, o_c4_1), axis=-1), dim * 8, ks, ks, ks, 1, 1, 1, 0.02, "SAME", "c4_3")

        o_c3_d = general_deconv3d(o_c4_3, dim*4, ks, ks, ks, 2, 2, 2, 0.02, "SAME", "c3_d")
        o_c3_3 = general_conv3d(tf.concat((o_c3_d, o_c3_1), axis=-1), dim * 4, ks, ks, ks, 1, 1, 1, 0.02, "SAME", "c3_3")

        o_c2_d = general_deconv3d(o_c3_3, dim*2, ks, ks, ks, 2, 2, 2, 0.02, "SAME", "c2_d")
        o_c2_3 = general_conv3d(tf.concat((o_c2_d, o_c2_1), axis=-1), dim*2, ks, ks, ks, 1, 1, 1, 0.02, "SAME", "c2_3")

        o_c1_d = general_deconv3d(o_c2_3, dim*1, ks, ks, ks, 2, 2, 2, 0.02, "SAME", "c1_d")
        o_c1_3 = general_conv3d(tf.concat((o_c1_d, o_c1_1), axis=-1), dim*1, ks, ks, ks, 1, 1, 1, 0.02, "SAME", "c1_3")
        o_c5_pad = tf.pad(o_c1_3, [[0, 0], [ks, ks], [ks, ks], [ks, ks], [0, 0]], "REFLECT")
        o_c0 = general_conv3d(o_c5_pad, img_layer, f, f, f, 1, 1, 1, 0.02, "VALID", "c0", do_relu=False)
        # Adding the tanh layer
        out_gen = tf.nn.tanh(o_c0, "t1")
        return out_gen
...
def build_classifier_old(input, dim, backbone='vgg3d', name="discriminator"):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE) as scope:
        if 'res18' in backbone:
            descs = build_feature_resnet18(input, dim, name='descs')
        else:
            descs = build_feature_vgg_3d(input, dim, name='descs')
        means = tf.get_variable("means", shape=descs.get_shape()[1::], dtype=tf.float32,
                                initializer=tf.zeros_initializer())
        descs = tf.nn.l2_normalize(descs - means, axis=-1)

    with tf.variable_scope('attention'):
        weights = tf.Variable(tf.ones([1, 4, 5, 4, 1], dtype=np.float32), name="weights", trainable=False)
        weights = tf.nn.relu(weights)
        weights = weights/tf.reduce_mean(weights)

    with tf.variable_scope(name, reuse=tf.AUTO_REUSE) as scope:
        # feats = tf.reduce_mean(descs, axis=(1, 2, 3))
        feats = tf.nn.l2_normalize(tf.reshape(descs*weights, (-1, descs.shape[1]*descs.shape[2]*descs.shape[3]*descs.shape[4])), axis=-1)
        logits, prob = fc_op(feats, "fc_layer", 2, activation=tf.nn.softmax)
        return logits, prob, feats


def build_classifier(input, dim, backbone='vgg3d', name="discriminator"):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE) as scope:
        ks = 3
        ...
        # descs = tf.nn.l2_normalize(tf.concat(o_p4, axis=-1), axis=-1)
